 ...
 */
static void tx_test(struct work_struct *unused,int test_size)
{
...
static int dma_proxy_probe(struct platform_device *pdev)
{
	 ...
	 */
	rc = create_channel(pdev, &channels[TX_CHANNEL], "h2c", DMA_MEM_TO_DEV);

	if (rc) 
		return rc;

	rc = create_channel(pdev, &channels[RX_CHANNEL], "c2h", DMA_DEV_TO_MEM);
	if (rc) 
		return rc;
